...
def sup_invoice():
    ...
    while i != 1:
        ...
        customer_name = input("Enter name of Supplier Name : ")
        with open('supplier.csv', 'r') as csvfile:

            reader = csv.DictReader(csvfile)
            for row in reader:
                if row['sup_name'] == customer_name:
                    customer_id = row['sup_id']
        total = 0
        ...
        with open('medicine.csv', 'r+') as csvfile, tempfile:
            ...
            columns = ['medi_name', 'med_id', 'sale', 'unit', 'quantity', 'min_quantity', 'comp_name', 'sup_id',
                       'to_pur']
            reader = csv.DictReader(csvfile)
            ...
            for row in reader:
                if row['medi_name'] == medi_name:
                    ...
                    if int(row['quantity']) < int(row['min_quantity']):
                        ...
                        row['to_pur'] = 0
                row = {'medi_name': row['medi_name'], 'med_id': row['med_id'], 'sale': row['sale'], 'unit': row['unit'],
                       'quantity': row['quantity'], \
                       'min_quantity': row['min_quantity'], 'comp_name': row['comp_name'], 'sup_id': row['sup_id'],
                       'to_pur': row['to_pur']}
                writer.writerow(row)
        shutil.move(tempfile.name, 'medicine.csv')

        with open('sales.csv', 'a+') as csvfile:
            columns = ['medi_name', 'med_id', 'sale', 'quantity', 'sale_date', 'sale_month', 'sale_year',
                       'customer_name', 'customer_id', 'total']
            writer = csv.DictWriter(csvfile, fieldnames=columns)

            writer.writerow(
                {'medi_name': medi_name, 'med_id': med_id, 'sale': sale, 'quantity': quantity, 'sale_date': date,
                 'sale_month': month, 'sale_year': year, 'customer_name': customer_name, 'customer_id': customer_id,
                 'total': total})

        print("Enter 0 for Suppling another medicine\nEnter 1 to print bill\n")
        i = int(input())
    ...
    for x in range(len(medicinename)):
        print("|", medicinename[x], "|", medicinequantity[x], "|", medicnecost[x], "|",
              medicinequantity[x] * medicnecost[x])
    print("|==============================|")
    print("|Grand Total===================|")
    grantotal = 0
    for x in range(len(medicinename)):
